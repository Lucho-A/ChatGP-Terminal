...
Features:
- option '--import-history' added. This option allows to import a history chat file (formatted as generated by '--csv-format'). 
The chats to be imported are the latest ones, and the number of them is defined by '--max-context-messages' (default 3). Take into account the openai restrictions, and the costs involved.  
