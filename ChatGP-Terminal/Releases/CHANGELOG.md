...
Features:
//TODO - option '--import-history' added. This option allows to import a history chat file (formatted as generated by '--csv-format'). The chats to be imported are the latest ones, and the number of them is defined by '--max-context-messages' (default 3). I think that, for the moment, the main objective of this feature could be the chance of continuing a previous session...anyway, just take into account the openai restrictions, and the costs involved.  
